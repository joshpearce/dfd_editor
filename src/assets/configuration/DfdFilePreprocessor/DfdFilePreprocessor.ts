...
export class DfdFilePreprocessor implements FilePreprocessor {
     ...
     * loads it.
     *
     * The backend (`transform.to_native`) now emits `data_item_refs` in the
     * OpenChart `ListProperty<StringProperty>` wire shape —
     * `[[syntheticKey, guidStr], ...]` — so no normalization is needed here.
     * Canvas `data_items` and all other properties already use the correct
     * `[[id, value], ...]` shape.  Legacy files (no `data_items` or
     * `data_item_refs`) pass through without error because the engine
     * defaults absent optional properties to empty `ListProperty`.
     *

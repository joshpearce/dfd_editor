...
- **Guarantees**:
  ...
  - `DfdCanvas` carries a `data_items` `ListProperty<DictionaryProperty>` keyed by item GUID; each sub-dict has required `identifier`, `name`, `classification` (closed enum `unclassified | pii | secret | public | internal`, default `"unclassified"`); optional `parent` (rendered via `PropertyType.DataItemParentRef` — a dropdown of process/data_store/external_entity blocks, with `null` meaning unowned) and optional `description`.

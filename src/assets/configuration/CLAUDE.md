...
- **Guarantees**:
  ...
  - `data_flow` template carries TWO `ListProperty<StringProperty>` properties — `node1_src_data_item_refs` and `node2_src_data_item_refs` (both default `[]`) — holding the GUIDs of data items flowing in each direction of the bidirectional Flow.

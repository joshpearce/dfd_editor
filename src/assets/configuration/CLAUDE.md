...
- **Guarantees**:
  ...
  - `DfdCanvas` carries a `data_items` `ListProperty<DictionaryProperty>` keyed by item GUID; each sub-dict has `parent`, `identifier`, `name`, `classification` (required; `classification` is a closed enum `unclassified | pii | secret | public | internal`, default `"unclassified"`) plus optional `description`.
